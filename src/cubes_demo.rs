...
pub fn plugin(app: &mut AppBuilder) {
	app.register_shader_uniforms::<CubeUniforms>()
		...
		.add_system(update_camera_position.system())
		.add_system(update_cube_positions.system())
		.add_system(update_spinning.system())
		.add_system_to_stage(CoreStage::PreRender, update_cube_uniforms.system());
}
...
fn spawn_cubes(
	...
	let orbiting_shader = shaders.add(Shader::new(shader::VERTEX, orbiting_shader::FRAGMENT));
	shader_meta.set(&orbiting_shader, &shader::TEXTURES, &shader::UNIFORMS);

	let core_shader = shaders.add(Shader::new(shader::VERTEX, core_shader::FRAGMENT));
	shader_meta.set(&core_shader, &shader::TEXTURES, &shader::UNIFORMS);
...
fn handle_camera_input(
	...
	mut motion_event: EventReader<MouseMove>,
	mut wheel_event: EventReader<MouseScroll>,
	window: Res<WindowSize>,
) {
	for MouseMove { dx, dy } in motion_event.iter() {
		let WindowSize { width, height } = *window;
		state.yaw -= (dx / width as f32) * 10.;
		let pitch_limit = 90.0f32.to_radians() * 0.99;
		state.pitch = (state.pitch - (dy / height as f32) * 10.).clamp(-pitch_limit, pitch_limit);
	}
	for MouseScroll { delta } in wheel_event.iter() {
		state.distance += 1.0 * if *delta > 0. { -1. } else { 1. };
	}
}
...
fn update_camera_position(
	...
	if state.is_changed() {
		if let Ok(mut transform) = q.single_mut() {
			...
			transform.translation =
				camera_rotation.transform_point3(vec3(0.0, 0.0, state.distance));
